...
// ─── Payment form (Stripe test) ───────────────────────────────────────────────
function PaymentForm({ intent, onPaid, onClose }) {
  ...
  return (
    <div style={s.overlay}>
      <div style={{ ...s.card, width: 380, maxWidth: "92vw", position: "relative" }}>
        ...
        <h3 style={{ fontWeight: 700, fontSize: 18, marginBottom: 6 }}>💳 Оплата</h3>
        <p style={{ fontSize: 13, color: "#94a3b8", marginBottom: 16 }}>
          Сумма: <strong style={{ color: "#fff" }}>${((intent.amount || 0) / 100).toFixed(2)} {(intent.currency || "usd").toUpperCase()}</strong><br />
          <span style={{ fontSize: 11 }}>Booking: {intent.bookingId}</span>
        </p>

        {done ? (
          <div style={{ padding: 16, borderRadius: 8, background: "#14532d", color: "#86efac", fontSize: 14, lineHeight: 1.6 }}>
            ✅ Платёж отправлен в Stripe (тестовый режим).<br />
            Статус обновится после получения webhook-события.
          </div>
        ) : (
          <form onSubmit={pay} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div>
              <label style={s.label}>Номер карты</label>
              <input style={s.input} value={card} onChange={e => setCard(e.target.value)} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ flex: 1 }}>
                <label style={s.label}>Срок</label>
                <input style={s.input} value={expiry} onChange={e => setExpiry(e.target.value)} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={s.label}>CVC</label>
                <input style={s.input} value={cvc} onChange={e => setCvc(e.target.value)} />
              </div>
            </div>
            <div style={{ padding: 10, borderRadius: 6, background: "#1e3a2f", border: "1px solid #166534", fontSize: 12, color: "#86efac" }}>
              🧪 Тест Stripe: карта <strong>4242 4242 4242 4242</strong> всегда проходит
            </div>
            <button style={{ ...s.btn, background: "#22c55e", fontWeight: 700 }} type="submit" disabled={loading}>
              {loading ? "Обработка..." : `Оплатить $${((intent.amount || 0) / 100).toFixed(2)}`}
            </button>
          </form>
        )}
      </div>
...
export default function App() {
  ...
  return (
    <div style={s.page}>
      <Toast toasts={toasts} />
      {paymentIntent && (
        <PaymentForm intent={paymentIntent} onPaid={handlePaid} onClose={() => setPaymentIntent(null)} />
      )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#64748b", marginTop: 3 }}>
            ...
            <span style={{ marginLeft: 8, fontSize: 11, background: "#1e293b", border: "1px solid #334155", borderRadius: 4, padding: "1px 6px", color: "#94a3b8" }}>
          ...
          {booking ? (
            <>
              ...
              {paymentIntent && booking.status === "PENDING" && (
                <button style={{ ...s.btn, background: "#6366f1", width: "100%", marginTop: 12, fontSize: 15 }}
                  onClick={() => {}}>
                  💳 Открыть форму оплаты
                </button>
              )}
              <button style={{ ...s.btn, background: "#1e293b", border: "1px solid #334155", width: "100%", marginTop: 10, fontSize: 13 }}
                onClick={async () => {
                  try {
                    const d = await apiFetch(`/payments/${booking.id}`, {}, accessToken);
                    setBooking(d.booking);
